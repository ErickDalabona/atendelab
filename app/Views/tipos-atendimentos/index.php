...
<script>
    ...
    async function carregarTipos() {
        try {
            ...
            if (!tipos.length) { tbody.innerHTML = '<tr><td colspan="4" class="text-center py-4">Nenhum tipo cadastrado.</td></tr>'; return; }
            tbody.innerHTML = tipos.map(tipo => `<tr>
                <td>${AtendeLabApi.escape(tipo.nome)}</td>
                <td>${AtendeLabApi.escape(tipo.descricao || '')}</td>
                <td><span class="badge ${tipo.status === 'ativo' ? 'text-bg-success' : 'text-bg-secondary'}">${AtendeLabApi.escape(tipo.status)}</span></td>
                <td class="text-end">
